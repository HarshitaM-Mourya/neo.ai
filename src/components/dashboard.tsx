...
const DUMMY_SESSIONS: ChatSession[] = [
    {
        ...
        messages: [
            { role: "neo", content: "Neutral networks initialized. I'm ready to capture the strategy discussion. Who's leading the core sync today? 🚀", timestamp: "10:00 AM" }
        ],
        attendees: ["Harsh", "Alex", "Neo AI"]
    },
    {
        id: "2",
        ...
        messages: [
            { role: "neo", content: "The dark evolution is complete. All 34 components are now harmonized. Ready for field testing? 💡", timestamp: "02:15 PM" }
        ]
    }
]
...
export function NeoDashboard() {
    ...
    const createNewSession = (isMeeting: boolean) => {
        const newSession: ChatSession = {
            ...
            messages: [{ role: "neo", content: "New vector initialized. Ready for input.", timestamp: "Just now" }],
    ...
    return (
        <div className="h-screen w-full overflow-hidden bg-[#050505] text-slate-300 font-sans selection:bg-blue-600/30">
            <CommandDialog open={isOpenCommand} onOpenChange={setIsOpenCommand}>
                <div className="bg-[#0A0A0A] border-slate-800 rounded-lg">
            ...
            <ResizablePanelGroup orientation="horizontal" className="h-full w-full">
                    ...
                    <div className="flex flex-col h-full shadow-2xl">
                        <div className="p-6 border-b border-white/5 bg-[#080808]/80 backdrop-blur-2xl sticky top-0 z-20">
                            <div className="flex items-center justify-between mb-5">
                                <h1 className="text-[11px] font-black tracking-[0.4em] text-white/40 uppercase">Intelligence Index</h1>
                                            ...
                                            <HoverCard key={s.id} openDelay={300}>
                                                <HoverCardTrigger asChild>
                                                    <button
                                                        ...
                                                        className={`w-full text-left px-4 py-4 rounded-2xl flex items-center gap-4 transition-all duration-500 relative group border-none ${
                                                    ...
                                                    >
                                                        <div className={`shrink-0 h-10 w-10 rounded-xl flex items-center justify-center transition-all ${
                                                            currentSessionId === s.id ? 'bg-blue-600 text-white' : 'bg-white/5 group-hover:bg-white/10'
                                                        }`}>
                                                            {s.isMeeting ? <Video className="h-4 w-4" /> : <MessageSquare className="h-4 w-4" />}
                                                        </div>
                                                        <div className="truncate flex-1">
                                                            <div className={`text-[13px] font-bold truncate leading-tight transition-colors ${currentSessionId === s.id ? 'text-white' : 'text-slate-400'}`}>{s.title}</div>
                ...
                {/* 3. RIGHT: MAIN STAGE & ANALYTICS */}
                <ResizablePanel defaultSize={76}>
                    <div className="flex h-full bg-[#050505] shadow-2xl">
                            ...
                            <Menubar className="h-16 border-b border-white/5 bg-[#050505]/80 backdrop-blur-3xl px-8 flex justify-between rounded-none border-t-0 border-x-0">
                                <div className="flex items-center gap-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[12px] font-black uppercase text-white tracking-[0.3em]">{currentSession?.title}</span>
                                    </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <TooltipProvider>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <Button variant="ghost" size="icon" className="h-10 w-10 text-slate-500 hover:text-white transition-all rounded-xl" onClick={() => createNewSession(isMeetingMode)}>
                                                    <Plus className="h-4 w-4" />
                                                </Button>
                                            </TooltipTrigger>
                                ...
                                <div className="max-w-4xl mx-auto space-y-16 pb-20">
                                    ...
                                    {currentSession?.messages.map((msg, i) => (
                                        <div key={i} className={`flex gap-8 group/msg ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                            <div className={`mt-2 h-11 w-11 rounded-2xl flex items-center justify-center flex-shrink-0 relative transition-transform group-hover/msg:scale-110 ${
                                                msg.role === 'neo' 
                                            ...
                                            <div className={`flex flex-col space-y-4 max-w-[85%] ${msg.role === 'user' ? 'items-end' : ''}`}>
                                                ...
                                                <Card className={`p-6 rounded-[2.5rem] border-none relative group overflow-hidden transition-all duration-500 shadow-2xl ${
                                                    msg.role === 'user' 
                        ...
                        {/* 3.2 INSIGHTS & TELEMETRY (FAR RIGHT) */}
                        <div className="w-[440px] flex flex-col bg-[#050505] shadow-2xl relative z-30">
